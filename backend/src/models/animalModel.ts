...
export const updateAnimal = async (id: number, name: string, species: string, age: number, breed: string, status: string, imageUrl: string | null) => {
    const result = await pool.query(
        'UPDATE animals SET name = $1, species = $2, age = $3, breed = $4, status = $5, image_url = $6 WHERE id = $7 RETURNING *',
        [name, species, age, breed, status, imageUrl, id]
    );
    return result.rows[0];
};
